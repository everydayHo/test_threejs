...
async function init() {
	...
	const textGeometry = new TextGeometry('THREE.js Interactive Motion ', {
		font,
		size: 0.5,
		depth: 0.1,
		bevelEnabled: true,
	...
	const spotLight = new THREE.SpotLight(0xffffff, 8, 30, Math.PI * 0.15, 0.2, 0.5);
	...
	spotLightFolder.add(spotLight, 'distance').min(1).max(30).step(0.01).name('distance');
	spotLightFolder.add(spotLight, 'decay').min(0).max(10).step(0.01).name('decay');
	...
	const unrealBloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 0.42, 1, 0);
